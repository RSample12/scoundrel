...
class GameEngine:
    def __init__(self):
        ...
        self.current_turn_potions = []  # Track potions used in current turn
        self.god_mode = False  # God mode flag
        
    def initialize_game(self, god_mode=False):
        """Initialize the game"""
        self.deck.build()
        self.deck.shuffle()
        self.player = Player()
        ...
        self.last_avoided = False
        self.game_over = False
        self.current_turn_potions = []
        self.god_mode = god_mode  # Set god mode
        self.draw_room()
        
    def draw_room(self):
        """Draw cards from the deck to fill the room"""
        # Reset potion usage for new turn
        self.current_turn_potions = []
    ...
    def avoid_room(self):
        ...
        self.current_turn_potions = []  # Reset potion usage for new turn
        self.draw_room()
        return True
    ...
    def choose_card(self, index):
        ...
        if card.type == CardType.WEAPON:
            ...
            if len(self.current_turn_potions) > 0:
            ...
            self.discard.add_card(card)
            self.current_turn_potions.append(card)
            ...
            if weapon_usable:
                ...
                damage_taken = 0 if self.god_mode else max(0, monster_damage - weapon_damage)  # No damage in god mode
                
                if not self.god_mode:  # Only take damage if not in god mode
                ...
                # Check if player died (won't happen in god mode)
                ...
                # Fight barehanded
                monster_damage = card.get_damage()
                damage_taken = 0 if self.god_mode else monster_damage  # No damage in god mode
                
                if not self.god_mode:  # Only take damage if not in god mode
                    self.player.take_damage(damage_taken)
                self.discard.add_card(card)
                
                result = f"Fought {card} barehanded. "
                if damage_taken > 0:
                    result += f"Took {damage_taken} damage. "
                else:
                    result += "Took no damage. "
